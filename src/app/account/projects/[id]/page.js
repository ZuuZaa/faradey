...
export default function Project() {
    ...
    const mainFunc=async () => {
        ...
        const fetchData = async () => {
            ...
            let response=await fetch(`http://89.40.2.200:3461/api/projects/get-project-details?${params.toString()}`,{
                method: 'GET',
                dataType: 'json',
                headers: {
...
const [ProjectNote, setNote] = useState("");
    ...
    let orderProject=async (e)=>{
      ...
      let removeProject=async (e)=>{
        ...
        try {
            const res = await fetch("http://89.40.2.200:3461/api/projects/remove-project", {
            method: "POST",
            headers: {
                'Accept': 'application/json, text/plain',
                'Content-Type': 'application/json;charset=UTF-8',
    ...
    let handleSubmit = async (e) => {
        ...
        try {
          const res = await fetch(`http://89.40.2.200:3461/api/projects/get-pdf?${params1.toString()}`, {
            method: "GET",
            headers: {
              'Accept': 'application/json, text/plain',
              'Content-Type': 'application/json;charset=UTF-8'
      ...
      let sendMail=async(e)=>{
        ...
        console.log(id)
        try {
            const res = await fetch(`http://89.40.2.200:3461/api/projects/send-mail-project`, {
              method: "POST",
              headers: {
                'Accept': 'application/json, text/plain',
                'Content-Type': 'application/json;charset=UTF-8'
      ...
      let changePrice=async(e)=>{
        ...
        console.log(option)
        let retail=document.getElementById("addp-main-table2").getElementsByClassName("retail_prc")
        let whole=document.getElementById("addp-main-table2").getElementsByClassName("whole_prc")
        let diller=document.getElementById("addp-main-table2").getElementsByClassName("diller_prc")
        if(option==1){
        ...
        try {
            const res = await fetch("http://89.40.2.200:3461/api/projects/change-prices-project", {
            method: "POST",
            headers: {
                'Accept': 'application/json, text/plain',
                'Content-Type': 'application/json;charset=UTF-8',
    ...
    return (
                        ...
                        <ol className="inline-flex items-center space-x-1 md:space-x-2 rtl:space-x-reverse">
                            ...
                            <li>
                                <div className="flex items-center">
                                    <a href="#" className="ms-1 text-sm font-medium md:ms-2">{t('Project Details')}</a>
                                </div>
                            </li>
      ...
                                        </div>
                                                ...
                                                <div className='projectdetail-info'>
                                                    <div className='mb-3'>
                                                        <h5>{t('Phone')}:</h5>
                                                        <input type='tel' className='border border-gray-200 w-full px-3 py-2 outline-none mb-2' defaultValue={phone} disabled />
                                                    </div>
                                                    <div className='mb-3'>
                                                        <h5>{t('Address')}:</h5>
                                                        <input type='text' className='border border-gray-200 w-full px-3 py-2 outline-none mb-2' defaultValue={city} disabled />
                                                    </div>
                                                    <div className='mb-3'>
                                                        <h5>{t('Email')}:</h5>
                                                        <input type='email' className='border border-gray-200 w-full px-3 py-2 outline-none mb-2' defaultValue={email} disabled />
                                                    </div>
                                                </div>
                                            </div>
                                                ...
                                                <thead className="bg-gray-50">
                                                    {/* {
                                                        properties==0?(<tr>
                                                            ...
                                                            <tr>
                                                                    <th scope="col" class="px-6 py-3 text-xs font-medium text-gray-500 uppercase tracking-wider">{t('Product name')}</th>
                                                                    <th scope="col" class="px-6 py-3 text-xs font-medium text-gray-500 uppercase tracking-wider retail_prc">{t('Price')}</th>
                                                                    <th scope="col" class="px-6 py-3 text-xs font-medium text-gray-500 uppercase tracking-wider whole_prc">{t('Wholesale Price')}</th>
                                                                    <th scope="col" class="px-6 py-3 text-xs font-medium text-gray-500 uppercase tracking-wider diller_prc">{t('Diller Price')}</th>
                                                                    <th scope="col" class="px-6 py-3 text-xs font-medium text-gray-500 uppercase tracking-wider">{t('Quantity')}</th>
                                                                    <th scope="col" class="px-6 py-3 text-xs font-medium text-gray-500 uppercase tracking-wider">{t('Total')}</th>
                                                                    <th scope="col" class="px-6 py-3 text-xs font-medium text-gray-500 uppercase tracking-wider">{t('Note')}</th>
                                                                    <th scope="col" class="px-6 py-3 text-xs font-medium text-gray-500 uppercase tracking-wider">{t('Remove')}</th>
                                                ...
                                                <tbody>
                                                    {
                                                        products.map(product=>{
                                                            ...
                                                            //     </td>
                                                            // </tr>):(
                                                                <tr className='border-b border-gray-200' id={product.productId}>
                                                                <td className="font-medium text-black hover:text-gray-700 transition ease-in">{product.productName}</td>
                                                                <td className="font-medium text-black hover:text-gray-700 transition duration-1 retail_prc">{product.price}₼</td>
                                                                <td className="font-medium text-black hover:text-gray-700 transition duration-1 whole_prc">{product.wholesalePrice}₼</td>
                                                                <td className="font-medium text-black hover:text-gray-700 transition duration-1 diller_prc">{product.dillerPrice}₼</td>
                                                                <td className="font-medium text-black hover:text-gray-700 transition duration-1"><input id="quant" type='number' style={{textAlign:"center",width:"100%"}} defaultValue={product.quantity}/></td>
                                                                <td className="font-medium text-black hover:text-gray-700 transition duration-1">{product.total}₼</td>
                                                                <td className="font-medium text-black hover:text-gray-700 transition duration-1"><input id="notee" type='text' style={{textAlign:"center",width:"100%"}} defaultValue={product.note}/></td>
                                                                <td className="font-medium text-black hover:text-gray-700 transition duration-1">
                                                                    <button className='font-bold inline-block text-base addp-projects-remove-btn rounded-full' onClick={removeProduct}>x</button>
                                                                </td>
                                                            </tr>
                                            ...
                                            <div className="grid grid-cols-1 md:grid-cols-2 gap-2 mt-5">
                                                <div>
                                                    <input type='text' className='border border-gray-200 w-full px-3 py-2 outline-none mb-2' placeholder={t('Text')} defaultValue={ProjectNote} onChange={(e) => setNote(e.target.value)}  />

                                                    <div className='flex gap-2 my-9'>
                                                            <select className='border border-gray-200 w-full px-3 py-2 outline-none mb-2' onChange={changePrice}>
                                            ...
                                            <div className='flex gap-2 justify-between'>
                                                        <button className='py-2 px-5 rounded-sm link-design1' onClick={saveProject}>{t('Save Project')}</button>
                                                        <button className='py-2 px-5 rounded-sm link-design2' onClick={removeProject}>{t('Remove Project')}</button>
                                            </div>
                                            <hr className='my-4'/>
                                            <div className='flex justify-between'>
                                                <div className='w-full md:w-3/4'>
                                                <div className='flex gap-2 justify-start mb-4'>
                                                        <Link href='' className='underline'onClick={handleSubmit}>{t('Download PDF')}</Link>
                                                    </div>
                                                    <div className='flex gap-2 justify-start mb-4'>
                                                        <Link href='' onClick={sendMail} className='underline'>{t('Send to email')}</Link>
                                                    </div>
                                                </div>
                                                <div className='w-full md:w-1/4 text-right'><button className='py-2 px-5 rounded-sm bg-yellow-400 hover:bg-yellow-600' onClick={orderProject}>{t('Order')}</button></div>
                                            </div>
                                        </div>
                                    </div>
                                </div>
