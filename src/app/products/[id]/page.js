...
export default function Products() {
  ...
  async function fetchData(){
    ...
    params.append('LanguageID',lang_id);
      ...
      const response=await fetch(`http://89.40.2.200:3461/api/products/get-index?${params.toString()}`,{
        method: "GET",
        headers: {
  ...
  let productFiltering=async()=>{
    ...
    try{
      const res = await fetch("http://89.40.2.200:3461/api/products/filtering", {
          method: "POST",
          headers: {
            'Accept': 'application/json, text/plain',
            'Content-Type': 'application/json;charset=UTF-8',
        ...
        <section className='category-main-section mt-5 pt-16'>
          ...
                    </div>):(<div className='info-icon absolute '>
            ...
                      </div>)
                            ...
                            </div>):(<div>{t('There is no product!')}</div>)
                            }
